const { Document, Packer, Table, TableRow, TableCell, TextRun, HeadingLevel, WidthType, AlignmentType, BorderStyle, ShadingType } = require('docx');
const fs = require('fs');
...
const HEADER_COLOR = '2F5496';

// Helper: tạo cell với styling
function createCell(text, options = {}) {
    const { bold = false, color = '000000', bgColor = null, width = null, align = AlignmentType.LEFT, colSpan = 1 } = options;
    ...
    cell.options = { ...cell.options, alignment: align };
    return cell;
}

// Helper: tạo header row
function createHeaderRow(headers, widths) {
    return new TableRow({
        children: headers.map((h, i) => createCell(h, { bold: true, bgColor: HEADER_COLOR, width: widths[i] })),
        tableHeader: true,
    });
}
...
async function createReport1() {
    ...
    const infoTable = new Table({
        ...
        rows: [
            new TableRow({ children: [createCell('Khach hang', { bold: true, bgColor: LIGHT_BLUE }), createCell('Cong ty CP KCN To hop Cong nghe Thanh Cong Viet Hung (TCVH)')] }),
            new TableRow({ children: [createCell('Nha thau', { bold: true, bgColor: LIGHT_BLUE }), createCell('3ASOFT')] }),
            new TableRow({ children: [createCell('San pham', { bold: true, bgColor: LIGHT_BLUE }), createCell('Arito ERP / Arito Procurement Suite')] }),
            new TableRow({ children: [createCell('Pham vi dot 1', { bold: true, bgColor: LIGHT_BLUE }), createCell('Mua noi dia + Nhap khau linh kien (L/C-SBLC, hai quan)')] }),
            new TableRow({ children: [createCell('Ngay chot pham vi', { bold: true, bgColor: LIGHT_BLUE }), createCell('29/06/2026')] }),
        ],
    });
    ...
    const painTable = new Table({
        width: { size: 100, type: WidthType.PERCENTAGE },
        rows: [
            createHeaderRow(['Ma Pain', 'Pain Point', 'He qua', 'Ma FR', 'Mo ta ma FR', 'Tinh thich ung'], [8, 15, 17, 10, 40, 10]),
            ...painPoints.map(row => new TableRow({ children: row.map((val, i) => createCell(val, { width: [8, 15, 17, 10, 40, 10][i] })) })),
        ],
    });
    ...
    const summaryTable = new Table({
        ...
        rows: summary.map((row, i) => new TableRow({ 
            children: row.map((val, j) => createCell(val, { bold: i === 4, bgColor: i === 4 ? LIGHT_BLUE : null })),
        })),
    });

    const children = [
        new TextRun({ text: 'BAO CAO 1', bold: true, size: 48, heading: HeadingLevel.TITLE }),
        new TextRun({ text: ' ', size: 24 }),
        new TextRun({ text: 'KHAO SAT TINH THICH UNG PHAN HE MUA HANG CUA NCC VOI NHU CAU QUAN TRI TCVH', bold: true, size: 32 }),
        new TextRun({ text: ' ', size: 24 }),
        new TextRun({ text: '1.1. Tong quan du an', bold: true, size: 28, heading: HeadingLevel.HEADING_2 }),
        new TextRun({ text: ' ', size: 20 }),
        infoTable,
        new TextRun({ text: ' ', size: 24 }),
        new TextRun({ text: '1.2. Diem dau hien tai cua TCVH', bold: true, size: 28, heading: HeadingLevel.HEADING_2 }),
        new TextRun({ text: ' ', size: 20 }),
        painTable,
        new TextRun({ text: ' ', size: 24 }),
        new TextRun({ text: '1.3. Tong hop danh gia tinh thich ung', bold: true, size: 28, heading: HeadingLevel.HEADING_2 }),
        new TextRun({ text: ' ', size: 20 }),
        summaryTable,
    ];
    ...
    const buffer = await Packer.toBuffer(doc1);
    fs.writeFileSync(OUTPUT_DIR + '\\BaoCao1_KhaoSatTinhThichUng.docx', buffer);
...
async function createReport2() {
    const children = [
        new TextRun({ text: 'BAO CAO 2', bold: true, size: 48, heading: HeadingLevel.TITLE }),
        new TextRun({ text: ' ', size: 24 }),
        new TextRun({ text: 'BANG TONG HOP RAI SOAT QUY TRINH HIEN TAI VA DE XUAT PHUONG AN MO RONG', bold: true, size: 28 }),
        new TextRun({ text: ' ', size: 24 }),
        new TextRun({ text: '2.1. Quy trinh mua sam noi dia', bold: true, size: 28, heading: HeadingLevel.HEADING_2 }),
    ];
    ...
    const flowTable = new Table({
        width: { size: 100, type: WidthType.PERCENTAGE },
        rows: [
            createHeaderRow(['Giai doan', 'Ma FR', 'Mo ta ma FR', 'PT moi', 'De xuat mo rong'], [10, 10, 45, 10, 25]),
            ...flowData.map(row => new TableRow({ children: row.map((val, i) => createCell(val, { width: [10, 10, 45, 10, 25][i] })) })),
        ],
    });

    children.push(flowTable);
    children.push(new TextRun({ text: ' ', size: 24 }));
    children.push(new TextRun({ text: '2.2. Quy trinh nhap khau linh kien (6 giai doan)', bold: true, size: 28, heading: HeadingLevel.HEADING_2 }));
    ...
    const nkTable = new Table({
        width: { size: 100, type: WidthType.PERCENTAGE },
        rows: [
            createHeaderRow(['Giai doan', 'Ma FR', 'Mo ta ma FR', 'PT moi', 'De xuat mo rong'], [15, 10, 40, 10, 25]),
            ...nkData.map(row => new TableRow({ children: row.map((val, i) => createCell(val, { width: [15, 10, 40, 10, 25][i] })) })),
        ],
    });

    children.push(nkTable);
    children.push(new TextRun({ text: ' ', size: 24 }));
    children.push(new TextRun({ text: '2.3. Cac yeu cau mo rong Giai doan 2', bold: true, size: 28, heading: HeadingLevel.HEADING_2 }));
    ...
    const wTable = new Table({
        width: { size: 100, type: WidthType.PERCENTAGE },
        rows: [
            createHeaderRow(['Ma', 'Ten phuong an', 'Mo ta chi tiet', 'Uu tien', 'Phu thuoc'], [8, 15, 47, 10, 20]),
            ...wData.map(row => new TableRow({ children: row.map((val, i) => createCell(val, { width: [8, 15, 47, 10, 20][i] })) })),
        ],
    });
    ...
    fs.writeFileSync(OUTPUT_DIR + '\\BaoCao2_RaiSoatQuyTrinh_DeXuatMoRong.docx', buffer);
...
async function createReport3() {
    const children = [
        new TextRun({ text: 'BAO CAO 3', bold: true, size: 48, heading: HeadingLevel.TITLE }),
        new TextRun({ text: ' ', size: 24 }),
        new TextRun({ text: 'KHAO SAT VA XAC NHAN YEU CAU PHAN HE MUA HANG NHA THAU', bold: true, size: 28 }),
    ];

    // Master Data
    children.push(new TextRun({ text: ' ', size: 24 }));
    children.push(new TextRun({ text: 'A. Master Data (Danh muc/Master)', bold: true, size: 28, heading: HeadingLevel.HEADING_2 }));
    ...
    const mdTable = new Table({
        width: { size: 100, type: WidthType.PERCENTAGE },
        rows: [
            createHeaderRow(['Ma', 'Ten yeu cau', 'Mo ta chi tiet', 'Uu tien', 'Nguon'], [10, 20, 45, 10, 15]),
            ...mdData.map(row => new TableRow({ children: row.map((val, i) => createCell(val, { width: [10, 20, 45, 10, 15][i] })) })),
        ],
    });
    ...
    // KHMS
    children.push(new TextRun({ text: ' ', size: 24 }));
    children.push(new TextRun({ text: 'B. Ke hoach mua sam (KHMS)', bold: true, size: 28, heading: HeadingLevel.HEADING_2 }));
    ...
    const khTable = new Table({
        width: { size: 100, type: WidthType.PERCENTAGE },
        rows: [
            createHeaderRow(['Ma', 'Ten yeu cau', 'Mo ta chi tiet', 'Uu tien', 'Nguon'], [10, 20, 45, 10, 15]),
            ...khData.map(row => new TableRow({ children: row.map((val, i) => createCell(val, { width: [10, 20, 45, 10, 15][i] })) })),
        ],
    });
    children.push(khTable);

    // YCMS
    children.push(new TextRun({ text: ' ', size: 24 }));
    children.push(new TextRun({ text: 'C. Yeu cau mua sam (YCMS)', bold: true, size: 28, heading: HeadingLevel.HEADING_2 }));
    ...
    const ycTable = new Table({
        width: { size: 100, type: WidthType.PERCENTAGE },
        rows: [
            createHeaderRow(['Ma', 'Ten yeu cau', 'Mo ta chi tiet', 'Uu tien', 'Nguon'], [10, 18, 47, 10, 15]),
            ...ycData.map(row => new TableRow({ children: row.map((val, i) => createCell(val, { width: [10, 18, 47, 10, 15][i] })) })),
        ],
    });
    children.push(ycTable);

    // Summary
    children.push(new TextRun({ text: ' ', size: 24 }));
    children.push(new TextRun({ text: '3.2. Tong hop yeu cau theo MoSCoW', bold: true, size: 28, heading: HeadingLevel.HEADING_2 }));
    ...
    const summaryTable = new Table({
        width: { size: 50, type: WidthType.PERCENTAGE },
        rows: summary.map((row, i) => new TableRow({ 
            children: row.map((val, j) => createCell(val, { bold: i === 5, bgColor: i === 5 ? LIGHT_BLUE : null })),
        })),
    });
    ...
    const doc2 = new Document({ sections: [{ properties: {}, children }] });
    const buffer = await Packer.toBuffer(doc2);
    fs.writeFileSync(OUTPUT_DIR + '\\BaoCao3_KhaoSatXacNhanYeuCau.docx', buffer);
