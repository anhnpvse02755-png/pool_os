...
class Supermemory {
    ...
    constructor({ baseURL = (0, env_1.readEnv)('SUPERMEMORY_BASE_URL'), apiKey = (0, env_1.readEnv)('SUPERMEMORY_API_KEY'), ...opts } = {}) {
        ...
        const customHeadersEnv = (0, env_1.readEnv)('SUPERMEMORY_CUSTOM_HEADERS');
        if (customHeadersEnv) {
            const parsed = {};
    ...
    static async local(options = {}) {
        ...
        const localBaseURL = baseURL || (0, env_1.readEnv)('SUPERMEMORY_LOCAL_URL') || `http://localhost:${port ?? (0, env_1.readEnv)('PORT') ?? 8787}`;
        if (start) {
        ...
        return new _a({
            ...clientOptions,
            apiKey: clientOptions.apiKey ?? (0, env_1.readEnv)('SUPERMEMORY_API_KEY') ?? 'local',
            baseURL: localBaseURL,
        });
    }
}
...
async function ensureLocalServer({ baseURL, port, version, timeout, fetch, }) {
    if (await isLocalServerReachable(baseURL, fetch))
        return;
    ...
    throw new Errors.SupermemoryError(`Timed out waiting for local Supermemory server at ${baseURL}. Try running \`npx supermemory local\` manually.`);
}
async function isLocalServerReachable(baseURL, fetch) {
...
async function startLocalServer({ port, version, }) {
    ...
    const [{ spawn }, cliPath] = await Promise.all([Promise.resolve().then(() => tslib_1.__importStar(require('node:child_process'))), resolveLocalCLIPath()]);
...
async function resolveLocalCLIPath() {
    ...
    const [{ existsSync }, pathModule, urlModule] = await Promise.all([
        Promise.resolve().then(() => tslib_1.__importStar(require('node:fs'))),
        Promise.resolve().then(() => tslib_1.__importStar(require('node:path'))),
        Promise.resolve().then(() => tslib_1.__importStar(require('node:url'))),
    ]);
