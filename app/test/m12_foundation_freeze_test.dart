...
void main() {
  ...
  final manifestFile = File(
    '$workspace/architecture/milestones/m12_freeze/contract_manifest.json',
  );
  final proofFile =
      File('$workspace/architecture/milestones/m12_freeze/proof_record.json');
  final manifest =
      jsonDecode(manifestFile.readAsStringSync()) as Map<String, dynamic>;
  final proof =
      jsonDecode(proofFile.readAsStringSync()) as Map<String, dynamic>;

  test('M12 inventory, symbols, dependencies, and cycles are frozen', () {
    final contracts =
  ...
  test('frozen M12 foundations have no hidden runtime mechanisms', () {
    final contracts =
        (manifest['contracts'] as List).cast<Map<String, dynamic>>();
    for (final contract in contracts) {
      final source = File('$workspace/app/lib/application/${contract['file']}')
  ...
  test('protected M3-M11 freeze artifacts remain unchanged', () {
    final protected =
        (proof['protectedArtifacts'] as Map).cast<String, String>();
    expect(proof['protectedM3M11'], 'unchanged');
