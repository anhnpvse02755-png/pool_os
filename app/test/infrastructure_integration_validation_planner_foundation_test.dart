...
void main() {
  ...
  test('infrastructure integration validation is deterministic and replay-safe',
      () {
  ...
  test('entries bind exact packaging deployment and readiness provenance', () {
    final plan = fixture.plan();
    expect(plan.packagingDeploymentAdapterPlanId,
        fixture.packagingDeploymentAdapterPlan.id);
    expect(
      plan.packagingDeploymentAdapterPlanDigest,
      fixture.packagingDeploymentAdapterPlan.digest,
    );
    expect(
        plan.productionReadinessProjectionId, fixture.productionReadiness.id);
    expect(
      plan.productionReadinessProjectionDigest,
      fixture.productionReadiness.digest,
    );
    for (var position = 0; position < plan.entries.length; position++) {
      final entry = plan.entries[position];
      final sourceEntry =
          fixture.packagingDeploymentAdapterPlan.entries[position];
      expect(entry.featureId, sourceEntry.featureId);
      expect(
        entry.packagingDeploymentAdapterEntryId,
        sourceEntry.packagingDeploymentAdapterEntryId,
      );
      ...
      expect(entry.productionReadinessProjectionDigest,
          fixture.productionReadiness.digest);
  ...
  test('canonical order ignores supplied collection order', () {
    ...
    final replay = InfrastructureIntegrationValidationPlan.create(
      packagingDeploymentAdapterPlan: fixture.packagingDeploymentAdapterPlan,
      productionReadinessProjection: fixture.productionReadiness,
  ...
  test('entries and exact structural log are immutable', () {
    ...
    expect(
      ...
      InfrastructureIntegrationValidationLogPhase.values,
    );
  });

  test('stale packaging deployment and readiness bindings fail closed', () {
    final source = fixture.plan();
    final stalePackaging = InfrastructureIntegrationValidationEntry.create(
      featureId: source.entries.first.featureId,
      packagingDeploymentAdapterEntryId:
          source.entries.first.packagingDeploymentAdapterEntryId,
      position: source.entries.first.position,
      packagingDeploymentAdapterPlanDigest: 'stale',
      productionReadinessProjectionDigest: fixture.productionReadiness.digest,
    );
    final staleReadiness = InfrastructureIntegrationValidationEntry.create(
      featureId: source.entries.first.featureId,
      packagingDeploymentAdapterEntryId:
          source.entries.first.packagingDeploymentAdapterEntryId,
      position: source.entries.first.position,
      packagingDeploymentAdapterPlanDigest:
          fixture.packagingDeploymentAdapterPlan.digest,
      productionReadinessProjectionDigest: 'stale',
    );
    for (final stale in [stalePackaging, staleReadiness]) {
      expect(
        () => InfrastructureIntegrationValidationPlan.create(
          packagingDeploymentAdapterPlan:
              fixture.packagingDeploymentAdapterPlan,
          productionReadinessProjection: fixture.productionReadiness,
  ...
  test('duplicate, orphan, and incomplete feature coverage fail closed', () {
    ...
    final orphan = InfrastructureIntegrationValidationEntry.create(
      featureId: source.entries.first.featureId,
      packagingDeploymentAdapterEntryId: 'orphan-packaging-deployment-adapter',
      position: source.entries.first.position,
      packagingDeploymentAdapterPlanDigest:
          fixture.packagingDeploymentAdapterPlan.digest,
      productionReadinessProjectionDigest: fixture.productionReadiness.digest,
  ...
  test('output excludes infrastructure mappings and inputs stay unchanged', () {
    final packaging = fixture.packagingDeploymentAdapterPlan.toJson();
    final readiness = fixture.productionReadiness.toJson();
    final json = fixture.plan().toJson().toString();
    expect(fixture.packagingDeploymentAdapterPlan.toJson(), packaging);
    expect(fixture.productionReadiness.toJson(), readiness);
    for (final forbidden in [
      'readinessEntryId',
      'deploymentGateId',
      'runtimeNodeId',
      'infrastructureOwnerId',
      'cloudResourceId',
      'healthCheckId',
      'startupId',
    ]) {
...
class _Fixture {
  const _Fixture({
    required this.packagingDeploymentAdapterPlan,
    required this.productionReadiness,
  });

  final PackagingDeploymentAdapterPlan packagingDeploymentAdapterPlan;
  final ProductionReadinessProjectionContract productionReadiness;

  InfrastructureIntegrationValidationPlan plan() =>
      const InfrastructureIntegrationValidationPlanner().plan(
        packagingDeploymentAdapterPlan: packagingDeploymentAdapterPlan,
        productionReadinessProjection: productionReadiness,
      );
}
