...
void main() {
  ...
  final manifestFile = File(
      '$workspace/architecture/milestones/m9_freeze/contract_manifest.json');
  final proofFile =
      File('$workspace/architecture/milestones/m9_freeze/proof_record.json');
  final manifest =
      jsonDecode(manifestFile.readAsStringSync()) as Map<String, dynamic>;
  final proof =
      jsonDecode(proofFile.readAsStringSync()) as Map<String, dynamic>;

  test('M9 inventory, symbols, dependencies, and cycles are frozen', () {
    final contracts =
    ...
    final edges = contracts
        .expand((contract) => (contract['dependencies'] as List).map(
              (dependency) => '${contract['file']}->$dependency',
            ))
        .toList();
    expect(manifest['milestone'], 'M9');
    expect(contracts, hasLength(8));
    expect(symbols, hasLength(28));
    expect(symbols.toSet(), hasLength(28));
    expect(files, hasLength(8));
    expect(edges, hasLength(10));
    expect(manifest['contractCount'], 8);
    expect(manifest['publicSymbols'], 28);
    expect(manifest['dependencyEdges'], 10);
  ...
  test('normalized contract hashes, versions, and set digest replay', () {
    final contracts =
        (manifest['contracts'] as List).cast<Map<String, dynamic>>();
    ...
    for (final contract in contracts) {
      ...
      expect(
        source,
        matches(RegExp(r'const \w+ContractVersion = 1;')),
      );
    }
  });
  ...
  test('frozen contracts have no hidden mutable or runtime mechanisms', () {
    final contracts =
        (manifest['contracts'] as List).cast<Map<String, dynamic>>();
    for (final contract in contracts) {
      final source = File('$workspace/app/lib/contracts/${contract['file']}')
  ...
  test('protected M3-M8 freeze artifacts remain unchanged', () {
    final protected =
        (proof['protectedArtifacts'] as Map).cast<String, String>();
    expect(proof['protectedM3M8'], 'unchanged');
