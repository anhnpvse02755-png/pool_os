...
void main() {
  ...
  test('transport adapter is deterministic and replay-safe', () {
  ...
  test('entries bind exact persistence and exposure provenance', () {
    final plan = fixture.plan();
    expect(plan.persistenceAdapterPlanId, fixture.persistenceAdapterPlan.id);
    expect(
      plan.persistenceAdapterPlanDigest,
      fixture.persistenceAdapterPlan.digest,
    );
    expect(plan.runtimeServiceExposureId, fixture.serviceExposure.id);
    expect(
      plan.runtimeServiceExposureDigest,
      fixture.serviceExposure.digest,
    );
    for (var position = 0; position < plan.entries.length; position++) {
      final entry = plan.entries[position];
      final configurationEntry =
          fixture.persistenceAdapterPlan.entries[position];
      ...
      expect(
          entry.runtimeServiceExposureDigest, fixture.serviceExposure.digest);
  ...
  test('canonical order ignores supplied collection order', () {
    ...
    final replay = TransportAdapterPlan.create(
      persistenceAdapterPlan: fixture.persistenceAdapterPlan,
      runtimeServiceExposure: fixture.serviceExposure,
  ...
  test('entries and exact structural log are immutable', () {
    ...
    expect(
      ...
      TransportAdapterLogPhase.values,
    );
  });

  test('stale persistence and exposure bindings fail closed', () {
    final source = fixture.plan();
    final staleConfiguration = TransportAdapterEntry.create(
      featureId: source.entries.first.featureId,
      persistenceAdapterEntryId: source.entries.first.persistenceAdapterEntryId,
      position: source.entries.first.position,
      persistenceAdapterPlanDigest: 'stale',
      runtimeServiceExposureDigest: fixture.serviceExposure.digest,
    );
    final staleFlutter = TransportAdapterEntry.create(
      featureId: source.entries.first.featureId,
      persistenceAdapterEntryId: source.entries.first.persistenceAdapterEntryId,
      position: source.entries.first.position,
      persistenceAdapterPlanDigest: fixture.persistenceAdapterPlan.digest,
      runtimeServiceExposureDigest: 'stale',
    );
    for (final stale in [staleConfiguration, staleFlutter]) {
      expect(
        () => TransportAdapterPlan.create(
          persistenceAdapterPlan: fixture.persistenceAdapterPlan,
          runtimeServiceExposure: fixture.serviceExposure,
  ...
  test('duplicate, orphan, and incomplete feature coverage fail closed', () {
    ...
    final orphan = TransportAdapterEntry.create(
      featureId: source.entries.first.featureId,
      persistenceAdapterEntryId: 'orphan-flutter-adapter',
      position: source.entries.first.position,
      persistenceAdapterPlanDigest: fixture.persistenceAdapterPlan.digest,
      runtimeServiceExposureDigest: fixture.serviceExposure.digest,
    );
  ...
  test('malformed structural log fails closed', () {
    ...
    expect(
      () => TransportAdapterPlan.create(
        ...
        log: List.filled(
          TransportAdapterLogPhase.values.length,
          source.log.first,
        ),
      ),
      throwsArgumentError,
  ...
  test('output excludes configuration values and inputs remain unchanged', () {
    final configuration = fixture.persistenceAdapterPlan.toJson();
    final flutter = fixture.serviceExposure.toJson();
    final json = fixture.plan().toJson().toString();
    expect(fixture.persistenceAdapterPlan.toJson(), configuration);
    expect(fixture.serviceExposure.toJson(), flutter);
...
class _Fixture {
  const _Fixture({
    required this.persistenceAdapterPlan,
    required this.serviceExposure,
  });

  final PersistenceAdapterPlan persistenceAdapterPlan;
  final RuntimeServiceExposureContract serviceExposure;

  TransportAdapterPlan plan() => const TransportAdapterPlanner().plan(
        persistenceAdapterPlan: persistenceAdapterPlan,
        runtimeServiceExposure: serviceExposure,
      );
}
