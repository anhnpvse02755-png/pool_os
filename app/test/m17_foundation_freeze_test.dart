...
void main() {
  ...
  final manifest = _json('$milestoneRoot/m17_freeze/contract_manifest.json');
  final proof = _json('$milestoneRoot/m17_freeze/proof_record.json');

  test('M17 planning inventory, sections, status, and dependencies are frozen',
      () {
    final artifacts =
        (manifest['artifacts'] as List).cast<Map<String, dynamic>>();
    final files = artifacts.map((artifact) => artifact['file'] as String).toSet();
    final sections = artifacts
        .expand((artifact) => (artifact['requiredSections'] as List).cast<String>())
        .toList();
    final edges =
        artifacts.expand((artifact) => artifact['dependencies'] as List).length;

    expect(manifest['milestone'], 'M17');
    expect(manifest['sourceRoot'], 'architecture/milestones');
    expect(artifacts, hasLength(9));
    expect(files, hasLength(9));
    expect(sections, hasLength(45));
    expect(edges, 27);
    expect(manifest['artifactCount'], 9);
    expect(manifest['requiredSections'], 45);
    expect(manifest['dependencyEdges'], 27);
  ...
  test('M17 freeze proof is canonical and deterministic', () {
    for (final key in [
  ...
  test('accepted M16 freeze and transitive M3-M16 chain remain unchanged', () {
    expect(proof['protectedM3M16'], 'unchanged');
    final protected =
