...
void main() {
  ...
  final manifestFile = File(
    '$workspace/architecture/milestones/m10_freeze/contract_manifest.json',
  );
  final proofFile =
      File('$workspace/architecture/milestones/m10_freeze/proof_record.json');
  final manifest =
      jsonDecode(manifestFile.readAsStringSync()) as Map<String, dynamic>;
  final proof =
      jsonDecode(proofFile.readAsStringSync()) as Map<String, dynamic>;

  test('M10 inventory, symbols, dependencies, and cycles are frozen', () {
    final contracts =
    ...
    final edges = contracts
        ...
        .toList();
    expect(manifest['milestone'], 'M10');
    expect(contracts, hasLength(8));
    expect(symbols, hasLength(27));
    expect(symbols.toSet(), hasLength(27));
    expect(files, hasLength(8));
    expect(edges, hasLength(7));
    expect(manifest['contractCount'], 8);
    expect(manifest['publicSymbols'], 27);
  ...
  test('normalized contract hashes, versions, and set digest replay', () {
    ...
    for (final contract in contracts) {
      ...
      expect(source, matches(RegExp(r'const \w+ContractVersion = 1;')));
    }
  });
  ...
  test('protected M3-M9 freeze artifacts remain unchanged', () {
    final protected =
        (proof['protectedArtifacts'] as Map).cast<String, String>();
    expect(proof['protectedM3M9'], 'unchanged');
