...
void main() {
  ...
  test('AI provider adapter is deterministic and replay-safe', () {
  ...
  test('entries bind exact transport and AI interaction provenance', () {
    final plan = fixture.plan();
    expect(plan.transportAdapterPlanId, fixture.transportAdapterPlan.id);
    expect(
      plan.transportAdapterPlanDigest,
      fixture.transportAdapterPlan.digest,
    );
    expect(plan.aiCoachInteractionSurfaceId, fixture.interactionSurface.id);
    expect(
      plan.aiCoachInteractionSurfaceDigest,
      fixture.interactionSurface.digest,
    );
    for (var position = 0; position < plan.entries.length; position++) {
      final entry = plan.entries[position];
      final configurationEntry = fixture.transportAdapterPlan.entries[position];
      ...
      expect(entry.aiCoachInteractionSurfaceDigest,
          fixture.interactionSurface.digest);
  ...
  test('canonical order ignores supplied collection order', () {
    ...
    final replay = AIProviderAdapterPlan.create(
      transportAdapterPlan: fixture.transportAdapterPlan,
      aiCoachInteractionSurface: fixture.interactionSurface,
  ...
  test('entries and exact structural log are immutable', () {
    ...
    expect(
      ...
      AIProviderAdapterLogPhase.values,
    );
  });

  test('stale transport and AI interaction bindings fail closed', () {
    final source = fixture.plan();
    final staleConfiguration = AIProviderAdapterEntry.create(
      featureId: source.entries.first.featureId,
      transportAdapterEntryId: source.entries.first.transportAdapterEntryId,
      position: source.entries.first.position,
      transportAdapterPlanDigest: 'stale',
      aiCoachInteractionSurfaceDigest: fixture.interactionSurface.digest,
    );
    final staleFlutter = AIProviderAdapterEntry.create(
      featureId: source.entries.first.featureId,
      transportAdapterEntryId: source.entries.first.transportAdapterEntryId,
      position: source.entries.first.position,
      transportAdapterPlanDigest: fixture.transportAdapterPlan.digest,
      aiCoachInteractionSurfaceDigest: 'stale',
    );
    for (final stale in [staleConfiguration, staleFlutter]) {
      expect(
        () => AIProviderAdapterPlan.create(
          transportAdapterPlan: fixture.transportAdapterPlan,
          aiCoachInteractionSurface: fixture.interactionSurface,
  ...
  test('duplicate, orphan, and incomplete feature coverage fail closed', () {
    ...
    final orphan = AIProviderAdapterEntry.create(
      featureId: source.entries.first.featureId,
      transportAdapterEntryId: 'orphan-flutter-adapter',
      position: source.entries.first.position,
      transportAdapterPlanDigest: fixture.transportAdapterPlan.digest,
      aiCoachInteractionSurfaceDigest: fixture.interactionSurface.digest,
    );
  ...
  test('malformed structural log fails closed', () {
    ...
    expect(
      () => AIProviderAdapterPlan.create(
        ...
        log: List.filled(
          AIProviderAdapterLogPhase.values.length,
          source.log.first,
        ),
      ),
      throwsArgumentError,
  ...
  test('output excludes AI mappings and inputs remain unchanged', () {
    final transport = fixture.transportAdapterPlan.toJson();
    final interaction = fixture.interactionSurface.toJson();
    final json = fixture.plan().toJson().toString();
    expect(fixture.transportAdapterPlan.toJson(), transport);
    expect(fixture.interactionSurface.toJson(), interaction);
...
class _Fixture {
  const _Fixture({
    required this.transportAdapterPlan,
    required this.interactionSurface,
  });

  final TransportAdapterPlan transportAdapterPlan;
  final AICoachInteractionSurfaceContract interactionSurface;

  AIProviderAdapterPlan plan() => const AIProviderAdapterPlanner().plan(
